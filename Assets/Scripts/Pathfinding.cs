...
public class Pathfinding : MonoBehaviour
{
    ...
    private List<GridPosition> CalculatePath(PathNode endNode)
    {
        ...
        {
            gridPositionList.Add(pathNode.NodeGridPosition);
        }
    ...
    private List<PathNode> GetNeighbourList(PathNode currentNode)
    {
        ...
        GridPosition gridPosition = currentNode.NodeGridPosition;
    ...
    public void Setup(int width, int height, float cellSize)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int z = 0; z < height; z++)
            {
                ...
                worldPosition = new Vector3(worldPosition.x+cellSize/2,0,worldPosition.z+cellSize/2);
                float raycastOffsetDistance = 1f;
                Debug.DrawRay(worldPosition + Vector3.down * raycastOffsetDistance, Vector3.up, Color.red, 10000f);
                if (Physics.Raycast(
    ...
    public List<GridPosition> FindPath(GridPosition startGridPosition, GridPosition endGridPosition, out int pathLength)
    {
        ...
        while (openList.Count > 0)
        {
            ...
            foreach (PathNode neighbourNode in GetNeighbourList(currentNode))
            {
                ...
                int tentativeGCost = currentNode.GCost + CalculateDistance(currentNode.NodeGridPosition, neighbourNode.NodeGridPosition);
                if (tentativeGCost < neighbourNode.GCost)
                {
                    neighbourNode.CameFromPathNode = currentNode;
                    neighbourNode.GCost = tentativeGCost;
                    neighbourNode.HCost = CalculateDistance(neighbourNode.NodeGridPosition, endGridPosition);
